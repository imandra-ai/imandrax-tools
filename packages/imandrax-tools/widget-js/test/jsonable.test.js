...
// The fixtures are the exact `yaml_str` traitlet -- real `to_yaml_str` output over
// real API data, generated by scripts/gen_widget_input_fixtures. They are what
// keeps the hand-rolled fold/highlight honest about what PyYAML actually emits.
...
describe("jsonable/view on real to_yaml_str output", () => {
  const TASKS = loadFixture("jsonable.tasks.iml"); // task entries, with a `|-` block
  const DECOMP = loadFixture("jsonable.decomp.iml"); // deeply nested region views

  it("takes the traitlet as a plain string", () => {
    expect(typeof TASKS).toBe("string");
    expect(typeof DECOMP).toBe("string");
  });

  it("shows every line of the dumper's output", () => {
    for (const yaml of [TASKS, DECOMP]) {
      const el = document.createElement("div");
      drawJsonable(el, yaml);
      const doc = el.querySelector(".imdx-jsonable-doc");
      for (const c of doc.querySelectorAll(".imdx-jsonable-count")) c.remove();
      // Line divs concatenate without separators; block bodies keep their \n.
  ...
  it("keeps a multi-line artifact repr in one block, not folded per line", () => {
    const el = document.createElement("div");
    drawJsonable(el, TASKS);
  ...
  it("nests the region tree, whose sequences sit at their key's indent", () => {
    const el = document.createElement("div");
    drawJsonable(el, DECOMP);
    const top = el.querySelector(".imdx-jsonable-fold");
    expect(top.querySelector("summary").textContent).toContain("- constraints:");
    // `children:` nests under the region item rather than becoming its sibling.
    expect(
      [...top.querySelectorAll(".imdx-jsonable-fold")].some((f) =>
        f.querySelector("summary").textContent.includes("children:"),
      ),
    ).toBe(true);
  });
});
