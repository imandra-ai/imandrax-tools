...
// Descendants of `selected` from relative depth 1 down to `maxDepth` — the tiles
// drawn inside the zoom root.
function visibleDescendants(selected: Rect, maxDepth: number): Rect[] {
  const out: Rect[] = [];
  const walk = (node: Rect, rel: number) => {
    if (rel > maxDepth) return;
    out.push(node);
    if (node.children) for (const c of node.children) walk(c as Rect, rel + 1);
  };
...
export function drawTreemap(el: HTMLElement, input: DrawInput, opts: TreemapOptions = {}): void {
  ...
  function renderTiles(): void {
    ...
    const visible = visibleDescendants(selected, cfg.maxDepth)
      .map((node) => ({ node, box: projectedBox(node, ext, vw, vh) }))
      .filter(({ box }) => box.width >= MIN_TILE_PX && box.height >= MIN_TILE_PX);

    select(tiles)
      .selectAll<HTMLDivElement, { node: Rect; box: Box }>(`div.${ROOT_CLASS}-tile`)
      .data(visible, (d) => labelPath(d.node))
      .join('div')
      .attr('class', `${ROOT_CLASS}-tile`)
      ...
      .style('background', (d) => color(d.node))
      .attr('title', (d) => tileTooltip(d.node))
      .html((d) => tileLabel(d.box) ? cellLabel(d.node) : '')
      .on('click', (_event, d) => {
        pickedEl = currentTarget(_event);
        showDetail(d.node);
      })
      .on('dblclick', (_event, d) => {
        if (zoomable(d.node)) setSelected(d.node);
      })
