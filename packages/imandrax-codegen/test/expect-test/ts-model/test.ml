    ...
    (fun (sub_dir, name) ->
      let code =
        [%string
          ...
          \    let sir_val_assignment = load_sir_model \"%{sub_dir}\" \
           \"%{name}\" in\n\
          \    let code = Typescript_adapter.Emit.emit_value_assignment \
           sir_val_assignment in\n\
          \      print_endline code;\n\
          \    [%expect {||}];\n\
          \    ;;\n\
          \    "]
...
let%expect_test "primitive/tuple_of_bool_and_int" =
  ...
  let code = Typescript_adapter.Emit.emit_value_assignment sir_val_assignment in
  print_endline code;
  [%expect {| const w: [boolean, number] = [true, 2]; |}]
;;
...
let%expect_test "primitive/record" =
  ...
  let code = Typescript_adapter.Emit.emit_value_assignment sir_val_assignment in
  print_endline code;
  [%expect {| const w: user = { id: 1, active: true }; |}]
;;
...
let%expect_test "primitive/variant1" =
  ...
  let code = Typescript_adapter.Emit.emit_value_assignment sir_val_assignment in
  print_endline code;
  [%expect {| const w: status = { tag: "Active", payload: null }; |}]
;;
...
let%expect_test "primitive/variant2" =
  ...
  let code = Typescript_adapter.Emit.emit_value_assignment sir_val_assignment in
  print_endline code;
  [%expect {| const w: status = { tag: "Waitlist", payload: 1 }; |}]
;;
...
let%expect_test "primitive/variant3" =
  ...
  let code = Typescript_adapter.Emit.emit_value_assignment sir_val_assignment in
  print_endline code;
  [%expect {| const w: status = { tag: "Waitlist", payload: [2, true] }; |}]
;;
...
let%expect_test "primitive/single_element_int_list" =
  let sir_val_assignment =
    load_sir_model "primitive" "single_element_int_list"
  in
  let code = Typescript_adapter.Emit.emit_value_assignment sir_val_assignment in
  print_endline code;
  [%expect {| const w: number[] = [1]; |}]
;;
...
let%expect_test "primitive/int_option" =
  ...
  let code = Typescript_adapter.Emit.emit_value_assignment sir_val_assignment in
  print_endline code;
  [%expect {| const w: number | null = { tag: "Some", payload: 2 }; |}]
;;
