...
let pp_term_view pp_t out (v : (_, _) Term.view) =
  ...
  | Term.Apply { f; l } ->
      fprintf
        out
        "Apply {@[<hv>@[<hv 2>f = %a@];@,@[<hv 2>l =@ @[<hv 1>[%a]@]@]@,@]}"
        pp_t
        f
        (pp_print_list ~pp_sep:(fun out () -> fprintf out ";@,") pp_t)
        l
  ...
  | Term.Tuple { l } ->
      fprintf
        out
        "@[<hv 2>Tuple@ {@,@[<hv 2>l =@ @[<hv 2>[%a]@]@]@,}@]"
        (pp_print_list ~pp_sep:(fun out () -> fprintf out ";@ ") pp_t)
        l
  | Term.Field { f; t } ->
      fprintf
        out
        "@[<hv 2>Field@ {@,@[<hv 2>f =@ %a@];@,@[<hv 2>t =@ %a@]@,}@]"
        (Imandrax_api_common.Applied_symbol.pp_t_poly pp_type)
        f
        pp_t
        t
  | Term.Tuple_field { i; t } ->
      fprintf
        out
        "@[<hv 2>Tuple_field@ {@,@[<hv 2>i =@ %d@];@,@[<hv 2>t =@ %a@]@,}@]"
        i
        pp_t
        t
