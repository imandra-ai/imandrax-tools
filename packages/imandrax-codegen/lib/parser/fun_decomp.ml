...
type model = { name : string; ty : Sir.type_expr; tm : Sir.value }
  ...
  | { f_id = Uid.{ name = f_id_name; view = _ }; f_args = _; regions } ->
      ...
      let (docstr_by_region : string list) =
        ...
        List.map2
          ...
          test_name_by_region docstr_body_by_region
      in

      zip5 test_name_by_region docstr_by_region models_by_region
        model_eval_expr_by_region model_eval_type_annot_by_region
      |> List.map
           (fun
             ( (test_name : string),
               (docstr : string),
           ...
           ->
             let f_args =
               models
               |> List.map (fun model -> (model.name, model.ty, model.tm))
             in
