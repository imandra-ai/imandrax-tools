...
and alias = { name : string; asname : string option }
...
and expr =
  ...
  | Dict of dict_expr
  | Compare of { left : expr; ops : cmpop list; comparators : expr list }

and constant = { value : constant_value; kind : string option }
...
and bool_op = And | Or
and bin_op = { left : expr; op : operator; right : expr }
...
and unary_op = Invert | Not | UAdd | USub
...
and list_comp = { elt : expr; generators : comprehension list }
and expr_context = Load | Store | Del
and list_expr = { elts : expr list; ctx : expr_context }
...
and name_expr = { id : string; ctx : expr_context }
and attribute_expr = { value : expr; attr : string; ctx : expr_context }
and subscript_expr = { value : expr; slice : expr; ctx : expr_context }
and call_expr = { func : expr; args : expr list; keywords : keyword list }
and lambda_expr = { args : arguments; body : expr }
and dict_expr = { keys : expr option list; values : expr list }
...
and stmt =
  ...
  (* 801 *)
  | Pass
  | Assert of { test : expr; msg : expr option }
...
and type_param =
  | TypeVar of type_var
  | ParamSepc of { name : string; default_value : expr option }
  | TypeVarTuple of { name : string; default_value : expr option }
