...
let rec parse_term (term : Term.term)
  ...
  | ( Term.Construct
    ...
    , { view =
      ...
      if not debug
      then ()
      else if List.length ty_view_constr_args <> 0
      then failwith "Never: LChar.t should have no args"
      else ();
  ...
  | ( Term.Construct
    ...
    *)
      if not debug
      then ()
      else if List.length ty_view_constr_args <> 1
      then (
        let args_str =
        ...
        let msg =
          sprintf
            "Never: list should have only 1 arg, got %d: %s"
