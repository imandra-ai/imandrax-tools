...
from .client import ImandraXAsyncClient, ImandraXClient
...
async def async_evaluate_term(
    client: ImandraXAsyncClient, iml: str, expr: str, po_skip: bool = True
):
    """
    Evaluate a given expression in the context of an IML snippet.
    ...
    if po_skip:
        ...
        task_filter = None
    body_eval_res = await client.eval_model(iml, task_filter=task_filter)
    ...
    leval_snippet = f'eval ({expr})'
    leval_res = await client.eval_src(leval_snippet)
    return body_eval_res, leval_res
